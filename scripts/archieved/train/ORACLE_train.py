...
def load_cached_protbert_batch(cache_dir: str | Path, headers: List[str], device="cuda"):
    cache_root = Path(cache_dir)
    rows = []
    for header in headers:
        path = cache_root / f"{header_to_stem(header)}.pt"
        if not path.exists():
            raise FileNotFoundError(f"Missing ProtBERT cache for {header}: {path}")
...
def infer_cached_protbert_dim(cache_dir: str | Path) -> int:
    root = Path(cache_dir)
    for p in sorted(root.glob("*.pt")):
        ...
        if not torch.is_tensor(vec):
            vec = torch.tensor(vec)
        if vec.ndim != 1:
            raise ValueError(f"Expected 1D cached ProtBERT vector, got shape={tuple(vec.shape)} from {p}")
        return int(vec.numel())
    raise ValueError(f"No cached ProtBERT vectors found in {root}")
...
# -----------------------
@torch.no_grad()
def evaluate(model, tokenizer, id2seq, pos_triples, batch_size=16, max_len=1024, device="cuda", sample_limit=200, protbert_cache_dir=None, id2name=None):
    """
    ...
    for i in range(0, len(triples), batch_size):
        ...
        if protbert_cache_dir:
            if id2name is None:
                raise ValueError("id2name is required when using protbert_cache_dir")
            headers = [id2name[pid] for pid in pids]
            cached = load_cached_protbert_batch(protbert_cache_dir, headers, device=device)
...
def train(args):
    ...
    use_cached_protbert = bool(getattr(args, "protbert_cache_dir", None))
    ...
    if use_cached_protbert:
        logger.info(f"Loading ProtBERT cache from: {args.protbert_cache_dir}")
        tokenizer = None
        cache_dim = infer_cached_protbert_dim(args.protbert_cache_dir)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_loader, start=1):
            ...
            if use_cached_protbert:
                ...
                cached = load_cached_protbert_batch(args.protbert_cache_dir, headers, device=device)
        ...
        metrics = evaluate(
            ...
            protbert_cache_dir=args.protbert_cache_dir if use_cached_protbert else None,
